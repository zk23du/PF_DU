...
def resnet20(**kwargs: Any) -> ResNet: # 18 = 2 + 2 * (2 + 2 + 2 + 2)
...
class ResNetCIFAR(nn.Module):
    def __init__(
        ...
        layers: List[int],
        num_classes: int = 100,  # Change the number of classes to 100
        has_bn: bool = True,
    ) -> None:
        ...
        if has_bn:
            ...
            self.bn1 = nn.Identity()
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.Identity()  # No max pooling for CIFAR-100

        self.layers = []
        self.layers.extend(self._make_layer(block, 64, layers[0], has_bn=has_bn))
        for num in range(1, len(layers)):
            self.layers.extend(self._make_layer(block, 64 * 2**num, layers[num], stride=2, has_bn=has_bn))
    ...
    def _make_layer(self, block: BasicBlock, planes: int, blocks: int, stride: int = 1, has_bn=True) -> List:
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            if has_bn:
                downsample = nn.Sequential(
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, has_bn=has_bn))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, has_bn=has_bn))
...
def resnet152_cifar(**kwargs: Any) -> ResNetCIFAR:
    return ResNetCIFAR(Bottleneck, [3, 8, 36, 3], **kwargs)

def resnet101_cifar(**kwargs: Any) -> ResNetCIFAR:
