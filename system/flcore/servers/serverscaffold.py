...
import time
import torch
from flcore.clients.clientscaffold import clientSCAFFOLDFW
from flcore.servers.serverbase import Server
from threading import Thread


class SCAFFOLDDFW(Server):
    def __init__(self, args, times):
        super().__init__(args, times)

        # select slow clients
        self.set_slow_clients()
        self.set_clients(clientSCAFFOLDFW)
    ...
    def train(self):
        ...
        if self.num_new_clients > 0:
            ...
            self.set_new_clients(clientSCAFFOLDFW)
            print(f"\n-------------Fine tuning round-------------")
